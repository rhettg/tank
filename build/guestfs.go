...
func EnsureGuestfsAppliance(progress io.Writer) (string, error) {
	...
	if hasAppliance(cacheDir) {
		ui.PrintStep(progress, "Guestfs appliance cached %s", ui.MutedStyle.Render(cacheDir))
		return cacheDir, nil
	}

	if systemDir := systemApplianceDir(); systemDir != "" {
		ui.PrintStep(progress, "Guestfs appliance system %s", ui.MutedStyle.Render(systemDir))
	...
	if !kernelReadable() {
		ui.PrintStep(progress, "Host kernel unreadable; skipping local build and downloading prebuilt appliance")
		if err := downloadAppliance(version, cacheDir, progress); err != nil {
			return "", err
		}
		ui.PrintSuccess(progress, "Guestfs appliance downloaded %s", ui.MutedStyle.Render(cacheDir))
		return cacheDir, nil
	}

	ui.PrintStep(progress, "Guestfs appliance missing; attempting local build")

	if _, err := exec.LookPath("libguestfs-make-fixed-appliance"); err == nil {
		ui.PrintStep(progress, "Building guestfs appliance")
		if err := buildFixedAppliance(cacheDir, progress); err == nil {
			ui.PrintSuccess(progress, "Guestfs appliance built %s", ui.MutedStyle.Render(cacheDir))
			return cacheDir, nil
		}
		ui.PrintStep(progress, "Failed to build guestfs appliance, falling back to prebuilt download")
	} else {
		ui.PrintStep(progress, "libguestfs-make-fixed-appliance not available, falling back to prebuilt download")
	}

	ui.PrintStep(progress, "Downloading guestfs appliance")
	if err := downloadAppliance(version, cacheDir, progress); err != nil {
		ui.PrintStep(progress, "Download failed (%v); falling back to supermin appliance", err)
		return "", nil
	}
	ui.PrintSuccess(progress, "Guestfs appliance downloaded %s", ui.MutedStyle.Render(cacheDir))
	return cacheDir, nil
